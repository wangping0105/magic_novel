...
module Crawler
  class Fulibus
    ...
    class << self
      ...
      private
      def deal_page(page)
        all_a_elements = page.search(".//article/a")
        all_a_elements.each do |a|
          detail_page = @agent.click(a)

          title = detail_page.search(".//*[@class='article-title']").text
          content = detail_page.search(".//*[@class='article-content']").children.to_html
          post_date = detail_page.search(".//*[@class='article-meta']/span").first.text
          category = detail_page.search(".//*[@class='article-meta']//a").first.text

          ht = Homeland::Topic.find_by(title: title)
          node = Homeland::Node.find_by(name: category)
      ...
      def check_next_page?(page)
        next_page = page.search(".//*[@class='pagination']//*[@class='next-page']/a").first
